...
#' Update schema-driven widget labels and choices for a new language
#' @param session Shiny session
#' @param schema Loaded schema
#' @param event_type Current event type const
#' @param subtype Current subtype const (or NULL)
#' @param language Language code or column name
#' @param input Current input values (for preserving selections)
update_schema_labels <- function(session, schema, event_type, subtype, 
                                  language, input) {
  iso <- lang_to_iso(language)
  pr <- schema$property_registry
  er <- schema$event_registry
  ...
  # Update event type selector choices
  ...
  # Update all event type properties (even non-visible; simpler than tracking)
  for (ec in names(er)) {
    event_entry <- er[[ec]]
    for (pn in event_entry$property_names) {
    ...
    if (event_entry$has_subtypes) {
      for (sc in names(event_entry$subtypes)) {
        ...
        for (spn in sub$property_names) {
          ...
          if (is.null(sdesc)) next
          if (sdesc$type == "const" || sdesc$type == "dataTable") next
